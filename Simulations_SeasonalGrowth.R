...
for(s in 1:no.sims){
  ...
  #Create dataframe with info about the first capture of each frog:
...
#--------------- Plot predicted growth curves based on model estimates -------------------------#
  ...
  l13 <- length(yrdays)
  #Setting metamorphosis date:
  stdate.s13 <- as.Date('2013-07-01')
  stdate.f13 <- as.Date('2013-08-01')  
  #would have used 1 Sep start, except then the time to 40mm is almost identical between two models and it's difficult to plot
  enddate <- as.Date('2015-01-01')
  #converting metamorphosis date to fraction of a year
...
#---Plot difference in time to sexual maturity (40mm) and 90% of asymptotic size -- 1 Aug emergence:
  ...
  #Calculating time difference to 90% aymptotic size
  #Constant model underestimates time
  ...
  #calculating time difference to sexuality maturity (40 mm)
  #constant model overestimates time
